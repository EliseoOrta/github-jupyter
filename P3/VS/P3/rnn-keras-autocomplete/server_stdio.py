...
def _complete(prefix: str, max_new: int = 360, temperature: float = 0.5) -> str:
    ids = _encode(prefix)
    if not ids:
        ids = [0]
    ...
    for _ in range(max_new):
        ...
        siguiente = int(rng.choice(len(probs), p=probs))
        ids.append(siguiente)

        caracter = _itos[siguiente]
        generado += caracter
        ...
        if msg.get("method") == "complete":
            return {
                **base,
                ...
                "text": _complete(
                    msg.get("prefix", ""),
                    int(msg.get("max_new", 360)),
                    float(msg.get("temperature", 0.5)),
                ),
            }
